...
BEGIN
	IF :CHOFER ='' THEN
		...
		--EDIR."StreetS" || '-' || EDIR."BlockS" || '-' || EDIR."CityS" || '-' || EDIR."CountyS"
		'' AS "Direccion" ,
		...
		(SELECT ED."DocEntry", IFNULL(SUM(IFNULL(ED."Quantity",0)*IFNULL("IWeight1",0)),0) "Peso",
		 ...
		 FROM INV1 ED INNER JOIN OITM ON OITM."ItemCode" = ED."ItemCode" GROUP BY ED."DocEntry") CD1
		 ON CD."DocEntry" = CD1."DocEntry"
		LEFT JOIN (SELECT "CardCode" ,"Address","ZipCode","City","County","Block","Street","U_VIS_SlpCode" 
		FROM CRD1 WHERE "AdresType"='S') DX ON
		...
		INNER JOIN
			(
				...
				WHERE
				"USER_CODE" LIKE :USUARIO
				ORDER BY OU."Name"
			) SER ON SER."U_SYP_NDSD" = CD."U_VIST_SUCUSU"
		LEFT JOIN (SELECT 
